...
def make_mind_map(user_input):
    response = openai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Consider the following text's (given by the user) main topics, brief descriptions of those topics, and connections between the topics. Create for me two outputs in a code block. The first is a dictionary where the keys are the main topic titles and the values are the descriptions. Keep both brief (the title should be a few words, the description should be a few sentences). The second is a list of tuples where each tuple contains two topics, representing the fact that those topics are connected in some way. In the code, the dictionary should be called dict and the tuple should be called tup. Say NOTHING ELSE, IT IS OF VITAL IMPORTANCE THAT YOU SAY NOTHING ELSE:"
                },
                {
                    "role": "user",
                    "content": user_input
    ...
    os.makedirs(uploads_dir, exist_ok=True)  # Ensure the "uploads" folder exists
    mind_map_path = os.path.join(uploads_dir, "mind_map.html")
